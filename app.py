...
# ==============================================================================
# 版本：v4.29 (Tab4 3D Full Upgrade)
# 日期：2026-02-24
# 狀態：正式發布版 (Production Ready)
#
# [版本歷程]
# v4.29 (2026-02-24) - Tab4 3D Full Upgrade
#   1. [A] 機殼改為半透明(opacity=0.12)，內部PCB/元件一覽無遺。
#   2. [A] 鰭片熱梯度：底部橙色→中間銀灰→頂部冷藍，per-fin intensity 著色。
#   3. [A] 3D 尺寸標注：L/W/H 直接標在模型邊緣。
#   4. [B] PCB 薄層(1.6mm 綠色)正確置於機殼底部。
#   5. [B] 元件熱點色塊：依 T_ref 冷→暖配色，hover 顯示 Tc/Tj 與 Margin。
#   6. [C] 視角預設按鈕：等角/頂視/前視/側視一鍵切換。
#   7. [C] 圖層顯示開關：機殼/PCB/元件熱點/鰭片/尺寸標注可獨立開關。
#
# v4.34 (2026-02-24) - Fixed-Design Tj_Margin + Guard Bug Fix
#   [Fix] compute_key_results: 移除錯誤的 Min_dT_Allowed < 50 guard，
#         改為 Bottleneck_Name != "None"（低環溫時 Allowed_dT > 50 被誤擋）。
#   [Fix] Tab5 Tj_Margin 改用 Fixed-Design 方式：固定基準散熱面積 (Area_base)，
#         計算各掃描點的溫度裕量 = Min_dT_Allowed - Total_Power/(h×Area_base×eff)。
#         意義：散熱器維持基準尺寸不變，參數改變時的實際溫度裕量變化。
#         T_amb +1°C → Tj_Margin 約 -1°C，Power Scale ×1.2 → Tj_Margin 明顯下降。
#   [Fix] Tornado Chart 同步改用 Fixed-Design Tj_Margin。
#
...
with tab_input:
    ...
    shared_column_config = {
        ...
        "Pad_L": st.column_config.NumberColumn("Pad 長 (mm)", help="IC E-PAD 長度或 PA 封裝底部長度或元件散熱端的長度 (mm)", format="%.2f"),
        "Pad_W": st.column_config.NumberColumn("Pad 寬 (mm)", help="IC E-PAD 寬度或 PA 封裝底部寬度或元件散熱端的寬度 (mm)", format="%.2f"),
...
# [Fix v4.31] 散熱器實際設計容量 = nominal_power × Margin，
# 故實際運作時基部溫升為 Min_dT_Allowed / Margin，Margin > 1 時元件自然有正裕度
T_hsk_base = T_amb + Min_dT_Allowed / Margin
    ...
    if not final_df.empty:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Pad_L": st.column_config.NumberColumn("Pad 長 (mm)", help="IC E-PAD 長度或 PA 封裝底部長度或元件散熱端的長度 (mm)", format="%.1f"),
                "Pad_W": st.column_config.NumberColumn("Pad 寬 (mm)", help="IC E-PAD 寬度或 PA 封裝底部寬度或元件散熱端的寬度 (mm)", format="%.1f"),
